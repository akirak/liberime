...
static emacs_value
get_config(emacs_env* env, ptrdiff_t nargs, emacs_value args[], void* data) {
  EmacsRime* rime = (EmacsRime*) data;
...
static emacs_value
set_config(emacs_env* env, ptrdiff_t nargs, emacs_value args[], void* data) {
  EmacsRime* rime = (EmacsRime*) data;
...
void liberime_init(emacs_env* env) {
  ...
  DEFUN("liberime-get-config", get_config, 2, 3, "get config test", rime);
  DEFUN("liberime-set-config", set_config, 3, 4, "set config test", rime);
}
